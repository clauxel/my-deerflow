...
await writeStaticPage('/pricing', {
  title: 'DeerFlow AI Pricing | Flow Annual Agent Workspace',
  description: 'Compare DeerFlow AI Lab, Flow, and Ops plans. Flow annual is selected by default and annual billing is 50% cheaper.',
  robots: 'index,follow',
...
await writeStaticPage('/privacy', {
  ...
  description: 'How DeerFlow AI handles analytics, checkout metadata, and managed-site interactions.',
  robots: 'index,follow',
  canonicalPath: '/privacy',
  rootHtml: buildLegalPrerender('Privacy Policy', 'This policy covers analytics, checkout, and user interactions on the managed DeerFlow AI site.'),
  structuredData: [],
})

await writeStaticPage('/terms', {
  title: `Terms | ${siteName}`,
  description: 'Terms for using the DeerFlow AI managed site, hosted payment flow, and related support services.',
  robots: 'index,follow',
  canonicalPath: '/terms',
  rootHtml: buildLegalPrerender('Terms of Service', 'These terms describe the limits and responsibilities of the managed DeerFlow AI site.'),
  structuredData: [],
})
...
await writeStaticPage('/checkout/done', {
  ...
  rootHtml: buildLegalPrerender('Finishing checkout...', 'You will return to the DeerFlow AI homepage when the hosted payment session closes.'),
  structuredData: [],
})
...
function renderHtml({ title, description, robots, canonicalPath, rootHtml, structuredData }) {
  ...
  if (graph) {
    html = html.replace(
      '</head>',
      `    <script type="application/ld+json" id="deerflow-prerender-schema">${JSON.stringify(graph)}</script>\n  </head>`,
    )
  }
  ...
  return `
    <main class="df-main">
      <section class="df-pricing-page-hero">
        ...
        <p class="df-lede">Flow annual is the default path and annual billing is 50% cheaper than the monthly run-rate.</p>
      </section>
    </main>`
}
