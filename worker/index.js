...
const LIVE_ORIGIN = 'https://deerflow.site'
const LIVE_HOST = 'deerflow.site'
const ALT_HOSTS = new Set(['www.deerflow.site'])
...
const planCatalog = {
  starter: {
    ...
    currency: 'USD',
    summary: 'one-mission DeerFlow AI pilot',
  },
  flow: {
    id: 'flow',
    name: 'Flow',
    ...
    currency: 'USD',
    summary: 'default managed DeerFlow AI workspace',
  },
  scale: {
    id: 'scale',
    name: 'Ops',
    ...
    currency: 'USD',
    summary: 'private runner and operational rollout planning',
  },
}
...
function resolveConfiguredProductId(env, planId, billing) {
  ...
  const tier = planId === 'scale' ? 'SCALE' : planId === 'starter' ? 'STARTER' : 'FLOW'
  const normalizedSelection = normalizeEnvKey(`${planId}_${billing}`)
  const keys = [
    `CREEM_PRODUCT_${tier}_${cycle}`,
    `CREEM_PRODUCT_ID_DEERFLOW_${normalizedSelection}`,
...
async function getOrCreateCreemProduct(env, apiKey, plan, billing, successUrl) {
  ...
  const product = await requestCreemJson(apiKey, `${resolveCreemBase(env)}/v1/products`, {
    name: `DeerFlow AI ${plan.name} (${billingLabel})`,
...
async function handleCheckout(request, env, requestUrl) {
  ...
  const planId = typeof body?.planId === 'string' ? body.planId : 'flow'
  const billing = body?.billing === 'monthly' ? 'monthly' : 'annual'
  const plan = planCatalog[planId] || planCatalog.flow
  ...
  try {
    ...
    const checkout = await requestCreemJson(apiKey, `${resolveCreemBase(env)}/v1/checkouts`, {
      ...
      request_id: `deerflow_${Date.now()}_${Math.random().toString(16).slice(2)}`,
      metadata: {
        site: 'deerflow.site',
        planId: plan.id,
        billing,
...
async function handleAnalytics(request) {
  ...
  console.log(JSON.stringify({ type: 'analytics', site: 'deerflow.site', accepted: events.length, events }))
